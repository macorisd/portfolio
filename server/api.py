...
def init_db():
    """
    Initialize Mongo client once (lazy) and return the db.
    Safe for serverless where startup hooks may not run.
    """
    ...
    try:
        mongodb_client = MongoClient(
            ...
            tlsCAFile=certifi.where(),  # robust TLS trust store for serverless
        )
        # Force a connection check so we can surface errors early
...
def get_db_or_503():
    database = init_db()
    if database is None:
        raise HTTPException(status_code=503, detail=f"DB unavailable: {INIT_ERROR}")
    return database
...
async def health_check():
    ...
    if database is None:
        raise HTTPException(
            status_code=503,
            detail=f"Mongo not ready. Last init error: {INIT_ERROR}"
        )
    try:
        mongodb_client.admin.command("ping")
        return {"status": "healthy", "database": "connected"}
    except ServerSelectionTimeoutError as e:
        raise HTTPException(status_code=503, detail=f"ServerSelectionTimeoutError: {repr(e)}")
    except PyMongoError as e:
        raise HTTPException(status_code=503, detail=f"PyMongoError: {repr(e)}")


@app.get("/education", response_model=List[Education])
async def get_education():
    """Get all education records ordered by end date (most recent first)"""
    database = get_db_or_503()
    try:
        collection = database.education
        education_data = []
...
async def get_work_experience():
    ...
    database = get_db_or_503()
    try:
        collection = database.work_experience
        work_data = []
...
async def get_index_data():
    ...
    database = get_db_or_503()
    try:
        collection = database['index-data']  # Use brackets for collection names with hyphens
        doc = collection.find_one()
